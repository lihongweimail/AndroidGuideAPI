...
def clean_all_entites_table():
    conn = pymysql.connect(host='localhost', port=3306, user='root',passwd='root', db='AndroidGuideAPI')
    cur = conn.cursor()
    cur.execute("delete from EntitiesMethod")
    cur.close()
    conn.commit()
    conn.close()
...
def insert_entity_todatabase(entities_schema_list):
    conn = pymysql.connect(host='localhost', port=3306, user='root',passwd='root', db='AndroidGuideAPI')
    cur = conn.cursor()
    cur.execute("select * from EntitiesMethod")
    i=cur.rowcount

    sqlInsertStr = "insert into EntitiesMethod values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
...
def insert_entities_from_method_list():
    ...
    for oneMethod in methodList:
        ...
        print("searching:"+str(j)+" / "+lenstr+"collecting : "+str(len(method_entities_list)) )
