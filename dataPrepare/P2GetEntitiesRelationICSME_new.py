...
def searching_in_entities_data(keywords1, keywords2,doc_text):
    conn = pymysql.connect(host='localhost', port=3306, user='root', passwd='root', db='AndroidGuideAPI')
    cur = conn.cursor()
    ...
    try:
        ...
        cur.execute("select DISTINCT  id, EntityName, EntitySection, EntityURL, EntityParent, EntityType, EntityOriginal, URLid, QualifiedName from EntitiesNew where (FIND_IN_SET(EntityName,%s)!=0 or FIND_IN_SET(EntityName,%s)!=0) ",(keywords1, keywords2))
        ...
        for row in results:
            ...
            if '.' in QualifiedName:
                # entitytype: Library, Classes, Interfaces, Exceptions, Annotations,Enums,Errors,Method,Field,Parameter
                if row[5] == 'Method' or row[5] == 'Field':
                    classname = QualifiedName.rsplit('.', 1)[1]
                elif row[5] == 'Parameter':
                    # use method name
                    classname = QualifiedName.rsplit('.', 1)[0]
                    classname = classname.rsplit('.', 1)[1]

                elif row[5] == 'Classes' or row[5] == 'Interfaces' or row[5] == 'Exceptions' or row[5] == 'Errors':
                    classname = row[1]
                elif row[5] == 'Library' or row[5] == 'Annotations' or row[5] == 'Enums':
                    classname = row[8]

            if classname not in doc_text:
                continue
...
def generate_id_data(id_list):
    conn = pymysql.connect(host='localhost', port=3306, user='root',passwd='root', db='AndroidGuideAPI')
    cur = conn.cursor()
    cur.execute('SELECT id FROM linkEntitysNew ORDER BY id DESC LIMIT 1')
    results = cur.fetchall()
    lastid=0
    for row in results:
        lastid = row[0]
    sqli = "insert into linkEntitysNew values(%s,%s,%s)"
    for i, one_schema in enumerate(id_list):
        cur.execute(sqli, (lastid+i + 1, one_schema[0],one_schema[1]))
    cur.close()
    conn.commit()
    conn.close()
    return
...
def triple_selection():
    ...
    entitidstringlist = set()

    schema_all_list = multiSelect(dict_allDoc, dict_allsentence, entitidstringlist, file_triple, filenameDict,schema_list)

    # generate_id_data(list(entitidstringlist))
    ...
    # print(schema_list)
    # remove duplicates
    schema_all_list=list(set(schema_all_list))

    print(len(schema_all_list))
    return schema_all_list


def multiSelect(dict_allDoc, dict_allsentence, entitidstringlist, file_triple, filenameDict,schema_list):
    ...
    for  j, one_triple in enumerate(file_triple):
        ...
        doc_text=dict_allDoc.get(URLid)
        ...
        searchingEntity1 = entity1.replace('(', ' ').replace(')', ' ').replace('[', ' ').replace(']', ' ').replace('<',' ').replace('>', ' ').replace('.', ' ').replace(' ', ',')

        searchingEntity2 = entity2.replace('(', ' ').replace(')', ' ').replace('[', ' ').replace(']', ' ').replace('<',' ').replace('>', ' ').replace('.', ' ').replace(' ', ',')

        entitylist = list(searching_in_entities_data(searchingEntity1, searchingEntity2, doc_text))

        # collect id already fetch from entities table
        onerunidset=set()
        for enid in range(0, len(entitylist)):
            entitidstringlist.add((str(entitylist[enid][0]), Relationid))
        ...
        if len(entitylist) > 0:
            for entity1i in range(0, len(entitylist)):
                ...
                linkentitylist.append((str(entitylist[entity1i][0]), Relationid))
        ...
        #keep the result for each step
        insert_generate_data(collectinglist)
        generate_id_data(linkentitylist)
...
def insert_generate_data(schema_list):
    ...
    cur.execute('SELECT id FROM EntitiesRelationBackup ORDER BY id DESC LIMIT 1')
    results = cur.fetchall()
    for row in results:
        lastid = row[0]


    sqli = "insert into EntitiesRelationBackup values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
    for i, one_schema in enumerate(schema_list):
        if len(one_schema[5])>3000:
            continue
        cur.execute(sqli, (lastid+i + 1, one_schema[0], one_schema[1], one_schema[2], one_schema[3], one_schema[4], one_schema[5], one_schema[6], one_schema[7], one_schema[8], one_schema[9], one_schema[10]))
    cur.close()
    conn.commit()
    conn.close()
    return
...
def main():
    ...
    schema_globe_list=triple_selection()

    #triple_selection()
    generate_data(schema_globe_list)
