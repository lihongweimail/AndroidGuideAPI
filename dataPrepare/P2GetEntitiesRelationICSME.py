...
def generate_id_data(id_list):
    conn = pymysql.connect(host='localhost', port=3306, user='root',passwd='root', db='AndroidGuideAPI')
    cur = conn.cursor()
    cur.execute("delete from linkEntitys")
    sqli = "insert into linkEntitys values(%s,%s,%s)"
...
def triple_selection():
    ...
    schema_all_list = multiSelect(collectingentityid, dict_allsentence, entitidstringlist, file_triple, filenameDict,schema_list)
...
def multiSelect(collectingentityid, dict_allsentence, entitidstringlist, file_triple, filenameDict,schema_list):
    ...
    for  j, one_triple in enumerate(file_triple):
        ...
        entitylist = list(searching_in_entities_data(searchingEntity1, searchingEntity2, collectingentityid))

        # collect id already fetch from entities table
        onerunidset=set()
        for enid in range(0, len(entitylist)):
            onerunidset.add(str(entitylist[enid][0]))


        RelationEntityIDs=''
        for ids in onerunidset:
            entitidstringlist.add((ids,Relationid))
            collectingentityid.add(ids)
